      ...
      <div className="relative mx-auto w-full max-w-[1400px]">
        ...
        <div className="mt-10 grid min-w-0 items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(260px,420px)_minmax(0,1fr)] lg:gap-8">
          <Reveal delay={0.05} variant="slideRight" className="min-w-0 lg:justify-self-end">
            ...
            <div className="mt-8 max-w-md" style={{ perspective: 900 }}>
          ...
          <Reveal delay={0.08} variant="scaleIn" className="flex min-w-0 justify-center">
            <ProcessOrbit activeIndex={activeIndex} />
          </Reveal>

          <Reveal delay={0.1} variant="slideLeft" className="min-w-0">
            <ul
              className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3 lg:grid-cols-2"
              role="tablist"
              aria-label="Build process steps"
            >
              {STEPS.map((step, i) => {
                const isActive = activeIndex === i;
                return (
                  <li key={step} role="presentation">
                    <button
                      type="button"
                      role="tab"
                      id={`process-tab-${i}`}
                      ...
                      className={`group w-full border-l py-1 pl-4 text-left transition-colors touch-manipulation ${
                        isActive
                          ? "border-portoAccent"
                    ...
                    >
                      <span
                        className={`font-sans text-[10px] tabular-nums transition-colors ${
                          isActive
                            ? "text-portoAccent"
                            : "text-neutral-600 group-hover:text-portoAccent"
                        }`}
                      >
            ...
            <p className="mt-6 font-sans text-[10px] uppercase tracking-[0.28em] text-neutral-600">
              Tap a step to explore — scroll freely
            </p>
          </Reveal>
        </div>
      </div>
